...
function getCurrentStock() {
    ...
    fetch('https://firestore.googleapis.com/v1/projects/e-commerce-base2/databases/(default)/documents/Inventory', {
        method: 'GET'
    })
    .then(response => response.json())
    .then(inventoryData => {
        const inventory = inventoryData.documents.map(item => {
            const name = item.fields.name?.stringValue ?? 'N/A'; // Use optional chaining to safely access properties
            const quantity = item.fields.quantity?.integerValue ?? 'N/A'; // Use optional chaining to safely access properties
            return {
                name,
                quantity
            };
